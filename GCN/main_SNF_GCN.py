...
import cross_val
from models_GCN_SNF import GCN
...
def evaluate(dataset, model_GCN, args, name='Test', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        adj_list = [adj[:,:,i] for i in range(adj.shape[2])]
        CBT_subject = snf.snf((adj_list), K=20)
...
def arg_parse():
    ...
    parser.add_argument('--dataset', type=str, default='LH_GSP',
                        help='Dataset')
    parser.add_argument('--fusion_method', type=str, default='SNF',
                        help='Fusion Method')
