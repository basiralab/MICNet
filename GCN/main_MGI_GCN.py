...
from models_GCN_MGI import GCN
...
def evaluate(dataset, model_GTN, model_GCN, args, name='Test', max_num_examples=None):
    model_GTN.eval()
    model_GCN.eval()
    tracked_Dict = {}
    labels = []
...
def train(args, train_dataset, val_dataset, model_GTN, model_GCN):
    Epoch_losses = []    
    GTN_losses = []
    params = list(model_GTN.parameters()) + list(model_GCN.parameters()) 
    ...
    for epoch in range(args.num_epochs):
        ...
        tracked_Dict = evaluate(val_dataset, model_GTN, model_GCN, args, name='Test', max_num_examples=100)
...
def benchmark_task(args):
    ...
    for i in range(vals):
        ...
        tracked_Dict = train(args, train_dataset, val_dataset, model_GTN, model_GCN)
        #labels.extend(label)
        #preds.extend(pred)
        tracked_dicts.append(tracked_Dict)
    return tracked_dicts
